...
if [[ $# -ge 1 ]]; then
  ...
  CFG="${ROOT}/configs/train/llamafactory_qwen25_7b_lora_dpo.yaml"
fi
...
if [[ ! -s "${ROOT}/artifacts/data/dpo.jsonl" ]]; then
  echo "Missing or empty ${ROOT}/artifacts/data/dpo.jsonl — run scripts/data/prepare.sh first."
  exit 1
fi
